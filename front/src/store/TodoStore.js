...
const TodoStore = create((set) => ({
    ...
    try {
      ...
      set((item) => {
        return {
          ...
          count: [...item.count, res.data.data],
        };
      }); // 업데이트
  ...
  get: async function (value) {
    const res = await axios.get(
      `${process.env.REACT_APP_APIURL}?sort=${value}`,
    );
  ...
  completeTodo: async function (id) {
    ...
    set(function (item) {
      ...
      return {
        data: updateData,
        count: item.count.map(function (obj) {
          if (obj._id == id) {
            obj.isdone = true;
          }
          return obj;
